...
- `salary_rules`：各工作模式的币种、最低和期望薪资；
- `industry_rules`：偏好或排除行业；
- `company_blacklists`：公司黑名单。

策略绑定一个候选人资料；同一用户的启用策略按优先级选择，职位决策保存策略和资料版本。
...
一条决策绑定职位、策略、候选人和解析版本，保存：

- `CONTACT / SKIP / REVIEW / FILTERED_OUT` 决策和置信度；
- `hard_rejected`、职位有效状态和动作阻断；
- 简短理由、匹配依据、不确定项和硬性排除原因；
- 输入指纹、决策/模型/提示版本和输入快照。

不再保存七维明细子表。`llm_invocations` 只追加模型调用元数据、输入哈希、状态和类型化错误。
...
会话绑定平台、招聘人、可空职位、当前策略/职位决策、资格状态、episode、终态证据和身份可靠性。
