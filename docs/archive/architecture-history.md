...
- `conversation_agent`：通过 Reply Router 按规则、知识库、LLM、等待 LLM 的顺序选择
...
- LLM 限流、认证/余额异常、超时、网络或服务不可用时打开用户级 LLM 能力熔断器，停止
  新 LLM 解析、评分、主动招呼和开放式回复，不再逐职位或逐消息调用模型。确定性硬过滤、
  Rule/Knowledge 回复、符合入站规则的明确简历请求和平台原生卡片、只读对账、租约回收
  及维护任务继续运行。熔断器只允许一个最小健康探测请求，按 5、10、20、40、60 分钟
  退避，第五次失败后固定每 60 分钟持续探测；成功后只恢复 LLM 相关队列。总览允许用户
  手动发起一次相同探测。
...
- `adapters/browser/job_discovery.py`：只负责读取当前唯一 BOSS 职位列表、按配置轮换
  “推荐、Java、区块链工程师”等搜索入口、驱动虚拟滚动；消息页和职位页均保持常驻，
  不主动刷新，
...
- 浏览器执行器必须在写入前记录可回读基线，并只把基线之后的新节点作为本次成功证据；
  输入框填充、按钮缺失和目标不唯一属于写入前失败，不能记为 `OUTCOME_UNKNOWN`。
- BOSS 回复、招呼、默认简历和平台同意卡片均通过原生 CDP 执行和回读；生产路径不再为
  简历发送附加 Playwright 浏览器对象。
- `OUTCOME_UNKNOWN` 对账使用短事务原子占用，对账期间不持有数据库行锁；Worker 启动循环
  会回收超时 `EXECUTING`，避免进程异常后永久占用。
- 真实数据库迁移遵循 `docs/database-operations.md`：迁移前冻结 Worker 和平台 Run、创建
  custom-format 备份并完成恢复演练，失败时恢复到独立数据库，不在原库盲目降级。

## 9. 第二阶段草稿流程

```text
入站消息 → 程序初步识别意图 → 更新 UNKNOWN / ROUGH_MATCH / FULL_MATCH / MISMATCH
→ Reply Router（Rule > Knowledge > LLM > Deferred）
→ 规则/知识库直接生成，未命中时才在存在有效职位评分和 LLM 时生成事实受限回复
→ MISMATCH 生成婉拒；否则检索当前有效知识项并继续沟通
→ 生成事实受限草稿或简历发送建议 → 程序执行权限与证据校验
→ 电话/面试具体时间创建 PENDING_APPROVAL；LLM 不可用且无法安全降级的普通消息等待恢复
```
...
R0 已确认上述能力分区熔断为目标架构。当前 Worker 仍采用“LLM 熔断后整轮停止”的旧
实现，平台原生卡片也尚未全部接入统一策略授权。在 R1 和 R3 完成并通过故障注入测试前，
真实平台 Run 必须保持暂停，且全局自动化保持紧急停止；只允许离线夹具、只读检查和数据库
备份验证。该临时冻结是运行安全措施，不是新的长期业务规则。
...
`REAL_CDP`，MOCK 只能使用 `FAKE`，并将实际类型写入运行记录。Worker 创建的临时职位
详情页只有在目标 ID 和具体职位 URL 均与本次创建记录一致时才允许关闭；用户原有的职位
列表页、消息页以及已发生导航变化的页面不得关闭或主动刷新。切换职位搜索入口后应在限定
时间内等待页面恢复，不能把加载中的短暂不可见直接判定为职位发现不可用。进程锁负责阻止本机
...
单条失败动作的调度异常不得中断整个平台周期：执行前异常延后该动作并继续队列，当前
策略已经拒绝的历史重试进入明确终态；动作进入执行后才发生异常则转为
`OUTCOME_UNKNOWN` 并暂停，禁止盲目重试。等价草稿必须复用发送指纹去重结果，不得因
草稿 ID 不同重复调度。
...
脉脉普通招聘私信仍进入统一会话流水线和消息中心；推荐卡片及官方通知不得因此被当成
普通私信重复入库。Telegram 频道帖子只产生职位记录，首次联系人私聊发送成功后才建立
实际会话并进入消息中心。
...
- 列表提取按卡片容错：单个坏卡片只记录诊断并跳过；页面有卡片但无任何可识别项时，
  判定页面结构变化并暂停。
- 消息优先使用平台消息 ID；缺失时使用会话、方向、平台时间和规范化正文生成稳定 ID。
  缺少平台 ID 与平台时间的派生身份不允许自动写入。
- 职位标题预筛只有 `RELEVANT/IRRELEVANT/UNKNOWN` 三态；只有确定无关才跳过，未知职位
  只打开一次详情后以 JD 结果去重。
- Worker 同一时刻只处理一个临时职位详情页；读取快照后立即关闭临时页，再进入可能较慢的
  LLM 流程。常驻消息页和职位列表页由页面角色注册表协调，目标不唯一时暂停而不猜测。
- 会话终态绑定消息证据和 episode。列表末条未变化时不再打开；终态后只有新岗位或明确重开
  表达才开启下一 episode。

## 15. R3 能力队列

Worker 每轮先运行不依赖 LLM 的发现、Rule/Knowledge Reply、明确卡片、简历请求、对账和
维护任务，再运行 LLM 解析、评分与开放式回复。LLM 熔断器只控制第二组能力；健康探测成功
只关闭模型熔断，不修改平台登录状态、用户暂停或 Worker 租约。

## 16. R4 查询与控制台边界

- 列表查询在 PostgreSQL 中完成筛选、稳定排序和分页，不以内存切片代替分页；
- 总览使用独立聚合查询，不复用职位、消息或动作列表的当前页数据；
- 控制台按 Run、Worker、平台会话和动作能力四层展示状态，并定时刷新；
- API 默认仅接受回环地址请求，非本机访问必须配置访问令牌；
- 前端页面继续使用路由级懒加载，避免把所有业务页面打入入口包。
