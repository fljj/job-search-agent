...
- `generated_drafts`：招呼或回复草稿、事实引用、置信度、风险、生成器版本和可空
  ...
  `RULE_TEMPLATE/KNOWLEDGE_BASE/LLM/HUMAN`。`HUMAN` 只表示用户主动编辑或人工发送的
...
- `automation_settings`：按 `GLOBAL/PLATFORM/STRATEGY` 保存开关、暂停状态及主动招呼阈值；
  `(user_id, scope_type, scope_key)` 唯一。旧低分婉拒、回复置信度和简历分数门槛字段已迁移删除。
...
- `automation_settings` 按全局、平台和策略范围保存开关、主动招呼门槛、去重冷却、
  工作时段和紧急停止，不保存小时或每日投递配额。
