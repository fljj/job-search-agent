...
- BOSS 职位发现按小批次串行处理，当前批次完成后才允许下一次扫描。出现 LLM 限流、
  超时或暂时不可用时停止当前批次，仅重试队首职位；成功或最终失败后仍等待正常批次
  间隔，再处理下一条。重试采用指数退避并设最大间隔和最大次数，目标不在当前列表也
  累计尝试次数，等待期间不发现新职位。迁移前缺少调度时间的历史重试不会驱动浏览器。
