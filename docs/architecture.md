...
前端以页签作为导航，但页面模块通过 `React.lazy` 动态加载，只有当前活动页面进入渲染树。
控制台只调用服务端聚合接口，不在浏览器内重新计算评分、自动化权限或动作状态。

- 职位页结构化展示模型版本、七维证据、模型主动沟通建议和程序 80 分授权；
- 对话页展示绑定策略、最新评分、低分婉拒草稿和简历动作结果；
- Agent 页展示脱敏 LLM 配置状态、运行心跳、限速计数、暂停原因和普通自动动作；
