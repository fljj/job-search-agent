...
浏览器 Worker、正式运行治理和排期确认；各阶段边界以 `development-plan.md` 为准。
...
简历反馈判断；当前正式运行使用智谱 GLM-5.2，所有模型输出仍需经过领域规则校验。
...
- `conversation_agent`：通过 Reply Router 按规则、知识库、LLM、人工待处理的顺序选择
  回复来源；从最近带方向的原始消息和成功动作即时构建结构化对话记忆，并在模型生成后
  确定性移除重复问题；规则与知识库层不依赖 LLM，生成结果仍交由策略决策和动作队列处理；
- `scoring`：先使用确定性解析和硬性过滤；命中时持久化排除原因并停止，不调用职位
  LLM 解析、评分或招呼语生成；通过后才进入完整解析和 LLM 评分；
- BOSS 职位发现按小批次串行处理，当前批次完成后才允许下一次扫描。
- LLM 限流、认证/余额异常、超时、网络或服务不可用时打开全局 LLM 熔断器，所有平台
  Worker 业务立即停止，不再逐职位或逐消息调用模型。熔断器只允许一个最小健康探测请求，
  按 5、10、20、40、60 分钟退避，第五次失败后固定每 60 分钟持续探测；成功后自动关闭
  熔断并在下一轮恢复业务。总览允许用户手动发起一次相同探测。
- 页面详情未就绪、评分输出格式错误等非全局 LLM 故障仍使用职位记录自身的有限重试。
  BOSS 和 Telegram 每轮只推进队首一条到期记录，目标不在当前列表也累计尝试次数；
  达到上限后明确结束，避免旧记录永久悬挂。迁移前缺少调度时间的历史重试不会驱动浏览器。
...
- `run_agent_worker.py`：串联全量会话消息发现与职位发现；已读但最后由招聘方发言的
  会话仍按消息证据和幂等记录处理。拥有稳定最后消息键的会话每条消息只尝试读取一次，
  新消息导致键变化后再处理；页面结构连续三轮不符合预期时才暂停对应平台运行，单次
  临时失败只记录健康状态并在下一轮重试。
...
- `packages/audit/runtime_logging.py`：将结构化运行事件轮转写入配置目录，并复用脱敏过滤器。
...
入站职位的硬性过滤和正式评分、会话短租约、消息幂等导入、游标和状态审计。每轮最多
分析一个尚未评分的入站职位；硬性排除不调用 LLM，规则通过后才进行 LLM 评分，且已有
会话的评分不得创建主动招呼。职位暂时不能唯一绑定时允许保留
...
BOSS 消息读取器还负责把平台原生的简历索取、联系方式交换和工作地点确认卡片提取为
结构化待同意项。
`message_discovery_service` 仅在资格为 `ROUGH_MATCH` 或 `FULL_MATCH` 且没有硬性
排除时创建已授权、可幂等执行的同意动作；该路径不经过 Reply Router 或 LLM。浏览器
适配器只能执行精确白名单提示并回读结果，不得自行判断岗位资格。纯平台状态通知只作为
忽略事件保存，不能触发文字回复。

BOSS 消息列表使用会话 ID、列表时间、受控预览和未读数生成变化指纹，只有指纹变化
才打开会话详情。列表项完全缺少变化信号时，只能在当前完整扫描轮次内去重，服务层
每六十分钟释放一次这类不稳定去重键进行兜底核对。状态为 `DECLINED`、`ENDED`、
`PAUSED` 或 `OUTCOME_UNKNOWN` 的会话在列表候选阶段直接排除，不再打开页面。

兜底核对重新打开会话时，应先从岗位卡片只读取得当前外部职位 ID。若该 ID 与会话已经
绑定且本地存在完整 JD，则直接复用本地职位记录，不再打开“查看职位”详情页；仅当职位
ID 变化、尚未绑定职位或本地 JD 缺失时打开详情。该复用不能跳过会话本身的新消息读取。
...
→ recommendation_service 在正式自动化双开关允许后原子创建唯一动作
...
Worker 仅在 MAIMAI 运行、全局/平台/策略自动化均启用且推荐开关开启时执行推荐动作。
同意动作还要求推荐简历开关；扫描、判断和动作执行通过稳定推荐 ID
