...
def evaluate_hard_filters(context: ScoringContext) -> list[RejectionReason]:
    ...
    if any(normalize_text(rule.pattern) in title for rule in strategy.title_rules
           if rule.rule_type == RuleType.EXCLUDE):
        reasons.append(_reason("TITLE_EXCLUDED", "职位名称命中排除规则", {"title": job.title}))

    mode_rule = next((rule for rule in strategy.work_mode_rules if rule.work_mode == job.work_mode), None)
    if job.work_mode != WorkMode.UNKNOWN and (mode_rule is None or not mode_rule.enabled):
        ...
        if location not in allowed:
            code = "ONSITE_LOCATION_NOT_ALLOWED" if job.work_mode == WorkMode.ONSITE else "HYBRID_LOCATION_NOT_ALLOWED"
            reasons.append(_reason(code, "工作地点不在策略允许范围",
                                   {"location": job.location, "allowed": mode_rule.allowed_locations}))

    owned = {normalize_skill(skill.name).lower() for skill in context.candidate.skills}
    core_required = {normalize_skill(skill).lower() for skill in strategy.core_required_skills}
    job_required = {normalize_skill(skill).lower() for skill in parsed.required_skills}
    relevant_core = core_required & job_required
    if relevant_core and relevant_core.isdisjoint(owned):
        reasons.append(_reason("REQUIRED_CORE_SKILL_MISSING", "候选人完全缺失职位核心硬门槛技能",
                               {"required_core_skills": sorted(relevant_core)}))

    salary_rule = next((rule for rule in strategy.salary_rules if rule.work_mode == job.work_mode), None)
    salary = parsed.salary
    if (salary_rule and salary and not salary.negotiable and salary.is_pre_tax is not False
            and salary.maximum_monthly_k is not None
            and salary.maximum_monthly_k < salary_rule.minimum_monthly_k):
        reasons.append(_reason("SALARY_BELOW_MINIMUM", "薪资上限低于策略最低接受值",
                               {"maximum_monthly_k": str(salary.maximum_monthly_k),
                                "minimum_monthly_k": str(salary_rule.minimum_monthly_k)}))

    industry = normalize_text(job.industry or "")
    excluded_industries = [rule.industry for rule in strategy.industry_rules
                           if rule.rule_type == IndustryRuleType.EXCLUDED
                           and industry and normalize_text(rule.industry) in industry]
    if excluded_industries:
        reasons.append(_reason("INDUSTRY_EXCLUDED", "职位行业被策略排除",
                               {"industries": excluded_industries}))

    company = normalize_company(job.company_name)
    if company in {normalize_company(item) for item in strategy.company_blacklist}:
        reasons.append(_reason("COMPANY_BLACKLISTED", "公司命中策略黑名单",
...
def _reason(code: str, message: str, evidence: dict[str, object] | None = None) -> RejectionReason:
    return RejectionReason(rule_code=code, message=message, evidence=evidence or {})
