...
面向 BOSS 直聘和脉脉的无人值守求职 Agent，并已加入猎聘 L3 只读消息与职位处理。BOSS
和脉脉可以处理普通招聘消息并按规则发送网站内已有简历；猎聘可发现消息、生成回复草稿、
创建电话/面试确认任务并发现和评分职位，但当前不会发送招呼、消息或简历。电话和面试的
...
- 当前只生成回复草稿和电话/面试确认任务，不会执行“聊一聊”“投简历”或消息回复
- Agent 只收起本轮自己打开的消息抽屉；用户原本打开的抽屉不会被关闭
...
5. 如需猎聘 L3，只保留唯一猎聘首页后选择 `LIEPIN` 启动；该 Run 先发现消息并准备草稿，
   再发现和评分职位，但不会执行平台写动作。
...
- BOSS 支持职位发现和消息处理；脉脉支持普通消息及系统推荐，暂不支持脉脉职位列表主动发现；
  猎聘当前支持消息发现、只读草稿准备、首页职位发现、硬过滤和评分，不支持平台写动作。
