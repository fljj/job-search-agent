...
按规则发送网站内已有简历；脉脉仅处理系统推荐卡片，不读取或回复普通私信；猎聘可发现消息和职位、判断是否沟通，并
执行“聊一聊”、普通回复及平台已有简历投递。电话和面试的
...
- 职位分析：先执行不消耗 Token 的硬性规则；命中后直接记录排除原因，只有通过的职位
  才进入必要的完整 JD 解析和 AI 沟通决策
- LLM 熔断：只暂停依赖 LLM 的解析、职位沟通决策、主动招呼和开放式回复；规则/知识库回复、
...
千问和智谱的结构化业务请求会显式关闭思考模式；职位沟通决策只发送精简 JD、相关履历和策略摘要，
编号并要求简洁输出；完整上下文仍保存在数据库用于硬过滤、
结果校验和审计，以降低延迟、Token 消耗和非流式请求超时概率。
决策契约只允许 `CONTACT / SKIP / REVIEW`；仅当决策 JSON 结构不合格时追加一次精简的
纠错请求，正常成功调用不会增加额外 Token。程序严格校验决策枚举、置信度、
证据和不确定项，不再计算或修正维度分数。
...
- Worker 会先读取首页消息抽屉，再发现、硬过滤和判断职位是否值得沟通；不主动刷新首页
- 新消息可普通回复或投递已登记的站内现有简历，
  满足主动门槛的职位可使用“聊一聊”
...
3. 开启主动招呼时，只有通过硬过滤且模型返回高置信度 `CONTACT` 的职位才会执行。
...
BOSS、脉脉和猎聘 Run 启用后直接按“系统设置”中的正式自动化配置运行，不需要初始化或升级
...
5. 如需猎聘，只保留唯一猎聘首页后选择 `LIEPIN` 启动；该 Run 先处理消息，再发现和判断职位。
...
清理运行历史会删除职位、沟通决策、会话、动作、审计、Worker 状态和排期数据；候选人资料、
...
- [职位沟通决策规则](docs/scoring-rules.md)
